...
class _CommonBottomSheetState extends State<CommonBottomSheet> {
  ...
  final List<CategoryItem> _categories = [
    CategoryItem(name: getLanguage().investment, icon: Icons.trending_up),
    CategoryItem(
        name: getLanguage().health, icon: Icons.medical_services_outlined),
    CategoryItem(name: getLanguage().bills_and_fees, icon: Icons.receipt_long),
    CategoryItem(name: getLanguage().food_or_drink, icon: Icons.restaurant),
    CategoryItem(name: getLanguage().car, icon: Icons.directions_car),
    CategoryItem(name: getLanguage().shopping, icon: Icons.shopping_cart),
    CategoryItem(name: getLanguage().gifts, icon: Icons.card_giftcard),
    CategoryItem(name: getLanguage().transport, icon: Icons.transit_enterexit),
  ];
            ...
            children: [
              Text(
                _transactionType.value == Transactions.income
                    ? getLanguage().income_amount.toUpperCase()
                    : getLanguage().expense_amount.toUpperCase(),
                style: TextStyle(
  ...
  Widget _buildTransactionTypeSelector() {
    return Obx(() => Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            _buildTypeButton(Transactions.income, getLanguage().income,
                Icons.arrow_upward_rounded),
            _buildTypeButton(Transactions.expense, getLanguage().expense,
  ...
  Widget _buildTypeButton(Transactions type, String label, IconData icon) {
    ...
    return GestureDetector(
      onTap: () {
        debugPrint('Đã chọn kiểu: $type');
        _transactionType.value = type; // Update the local observable
        controller.selectedType.value = type == Transactions.income
            ? 'income'
            : 'expense'; // Update the controller
        debugPrint(
            'Cập nhật controller.selectedType: ${controller.selectedType.value}');
      },
      ...
      children: [
        ...
        Container(
          ...
          child: TextField(
            ...
            cursorColor: Colors.white,
            decoration: InputDecoration(
              hintText: getLanguage().enter_transaction_name,
              hintStyle: TextStyle(
            ...
            child: Center(
              child: Text(
                controller.isLoading.isFalse
                    ? getLanguage().add_transaction
                    : getLanguage().processing,
                style: const TextStyle(
                  color: Colors.white,
                  fontWeight: FontWeight.w600,
                  fontSize: 18,
      ...
      builder: (context, child) {
        return Theme(
          data: ThemeData.dark().copyWith(
            ...
            dialogTheme:
                const DialogThemeData(backgroundColor: AppColor.darkBackground),
          ),
          child: child!,
        );
      },
